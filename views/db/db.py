...
class MedicalHistory(db.Model):
    ...
    diagnosis_id = db.Column(db.Integer, db.ForeignKey('diseases.disease_id'), nullable=False)
    diagnosis_date = db.Column(db.DateTime, nullable=False)
    user_response = db.Column(JSONB, nullable=False)
    model_accuracy = db.Column(db.Float, nullable=False)

    user = db.relationship('Users', backref='medical_histories')
    diagnosis = db.relationship('Diseases', backref='medical_histories')
